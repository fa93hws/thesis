...
This reduces the computational effort\hl{s} and leads to an efficient adaptive analysis.
...
An extremely small \hl{angle} as shown in Fig.~\ref{adap_fig:ei_mesh_quality} may raise numerical stability issues and hence decrease the accuracy of the result.
\begin{figure}[h!]
...
\paragraph{}
From Eq.~\ref{lr_eq:sbfem_displacement_field}, the displacement in the SBFEM can be calculated as
\begin{equation}
...
However, therms corresponding to the eigenvalue $\lambda_i > p$ indicate the shape functions are not capable to interpolate the displacement exactly and thus shall be taken as error.
