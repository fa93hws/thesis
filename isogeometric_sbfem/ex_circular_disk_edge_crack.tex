...
In the first example, consider a circular disk with an edge crack (see fig.~\ref{fig:circular_disk_geo_bc}) with Young’s modulus $E=1N/m^2$ and Poisson's ratio $\nu=0.3$.
    \begin{figure}[h!]
        \centering
        \scalebox{0.4}{
        ...
        \label{fig:circular_disk_geo_bc}
    \end{figure}
...
where $\kappa$ is the Kolosov constant defined in eq.~\ref{eq:kolosov_constant}.

The analytical stress solution for mode I are given by:
    \begin{subequations}
        \begin{align}
...
The control net and the location of control points are shown in fig.~\ref{fig:circular_disk_mesh} for different NURBS order.
    \begin{figure}
...
Eq.~\ref{eq:isosbfem_fracture_stress_field} is the parametric equation for the stress field in the polar coordinates $r$ and $\theta$.
The terms $\left(
        r_\eta^{\lambda_i+1}(\eta)
        \boldsymbol{\psi}_{\sigma_i}(\eta)
    \right)$
in eq.~\ref{eq:isosbfem_fracture_stress_field} together with $\theta(\eta)$ in eq.~\ref{eq:isosbfem_coordinate_transformation} are the stress modes describing the angular distribution at a constant radial coordinate $r$.
For the converged result, fig.~\ref{circular_disk_modes} shows the displacement and the stress distribution at a constant radial coordinate r around the crack tip for mode I fracture.
...
The convergence of the displacement and stress modes with h and p refinement is shown in Fig. 21.
It can be seen that with refinement the solution converges monotonically.
    ...
    \label{fig:circular_disk_convergence}
    \end{figure}
